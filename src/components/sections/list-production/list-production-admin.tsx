...
export default function ListProduction({ productions }: MyComponentProps) {
  ...
  function filterInput(e: string) {
    setResult(
      productions.filter(
        (prod) =>
          prod.producer.toLocaleLowerCase().includes(e.toLocaleLowerCase()) ||
          prod.product.toLocaleLowerCase().includes(e.toLocaleLowerCase())
      )
    );
  }
    ...
    <>
      <section id={style.SectionProducerList}>
        <StructContainer>
          ...
          >
            <label style={{ marginBottom: "8px" }} htmlFor="pesquisa">
              Pesquise pelo CPF do produtor ou produto:
            </label>
            <input
              style={{ borderRadius: "5px", padding: "6px" }}
              placeholder="CPF do produtor ou nome do produto"
              type="text"
              name="pesquisa"
              id="pesquisa"
              onChange={(e) => {
                setFilter(e.target.value), filterInput(e.target.value);
              }}
            />
          </div>
          <Row gutter={[12, 15]}>
              ...
              <>
                {
                  <>
                    {filter == "" ? (
                      productions.map(
                        ({ id, quantity, date, producer, product }, index) => (
                      ...
                      <>
                        {result!.length > 0 ? (
                          ...
                          <>
                            <h2 style={{ color: "red" }}>
                              Nenhum produtor ou produto encontrado!
                            </h2>
                          </>
                        )}
                      </>
